...
const ChatItem = ({ chat }: { chat: IResponse<any> }) => {
  if (chat.is_ai) {
    ...
    if (chat.is_mark_down && chat.is_video) {
      return (
        <div className="ai-chat">
          <Image
            width={28}
          ...
          <div>
            <div>
              <iframe
                ...
                src={`https://www.youtube.com/embed/${chat.data.iframe_url}?autoplay=1`}
                title="[OST Thưa Mẹ Con Đi / Goodbye Mother] Cảm Ơn Và Xin Lỗi - Chillies"
            ...
            <div>
              ...
              {chat.data.content_html ? (
                ...
                <ReactMarkdown>{chat.data.description}</ReactMarkdown>
              )}
            </div>
          </div>
        </div>
            ...
            {(chat.data as any[]).map((item, index) => {
              return (
                ...
                >
                  ...
                  <div>
                    {(item.image_url as string[]).map((item, index) => (
                      <div
                        key={index}
                        style={{
                          marginBottom: 30,
